(() => {
  ...
  function animate(now) {
    ...
    if (!dragging && visible) {
      if (Math.abs(velocityY) > 0.006 || Math.abs(velocityX) > 0.004) {
        rotY += velocityY * dt;
        rotX = clamp(rotX + velocityX * dt, -18, 18);
  ...
  stage.addEventListener('keydown', (e) => {
    ...
    else if (e.key === 'ArrowUp') rotX = clamp(rotX - 7, -18, 18);
    else if (e.key === 'ArrowDown') rotX = clamp(rotX + 7, -18, 18);
